    ...
    <section id="education" className="py-12 sm:py-16 lg:py-20 px-4">
      <div className="max-w-6xl mx-auto">
        <div className="text-center mb-8 sm:mb-12 lg:mb-16">
          ...
          <p className="text-gray-400 text-sm sm:text-base lg:text-lg">Academic excellence and continuous learning</p>
        </div>
        ...
        <div className="relative">
          ...
          {educationData.map((item, index) => (
            <div key={item.id} className={`relative flex flex-row lg:flex-row items-start mb-6 sm:mb-8 lg:mb-12 ${
              index % 2 === 0 ? 'lg:flex-row' : 'lg:flex-row-reverse'
            }`}>
              {/* Timeline Node */}
              <div className="lg:absolute lg:left-1/2 lg:transform lg:-translate-x-1/2 z-10 flex-shrink-0 mr-4 lg:mr-0">
                <div className={`w-10 h-10 sm:w-12 sm:h-12 lg:w-16 lg:h-16 rounded-full bg-gradient-to-r ${
                ...
                <div className="bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl p-3 sm:p-4 lg:p-6 hover:bg-slate-800/70 transition-all duration-300 hover:scale-105 hover:shadow-xl">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-2 sm:mb-3 gap-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                      item.color === 'blue' ? 'bg-blue-600/20 text-blue-400' :
                  ...
                  <h3 className="text-sm sm:text-base lg:text-xl font-bold text-white mb-1 sm:mb-2 leading-tight">{item.degree}</h3>
                  <h4 className="text-blue-400 font-semibold mb-1 text-xs sm:text-sm leading-tight">{item.institution}</h4>
                  <p className="text-gray-400 text-xs mb-1 sm:mb-2">{item.location}</p>
